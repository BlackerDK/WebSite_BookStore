 ...
 * @author DUY KHANH
 */
@WebServlet(name = "ServletSearchProducts", urlPatterns = {"/ServletSearchProducts"})
public class ServletSearchProducts extends HttpServlet {
private final String SEARCH_VALUE_PAGE="shop.jsp";
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String searchValue= request.getParameter("txtSearchValueProducts");//lấy thông tin từ login.html
        String url="login.jsp";
        try{
            if (!searchValue.trim().isEmpty()) {//Parameter never null !!
                //Call DAO
                BookDAO dao = new BookDAO();
                dao.searchProducts(searchValue);               
                List<Book> result = dao.searchProducts(searchValue);//Kết quả động
                url=SEARCH_VALUE_PAGE;//Cơ chế chuyển trang thành trang động jsp               
                request.setAttribute("SEARCH_RESULT_PRODUCT", result); //Chỉ show thôi không lưu
